					...
					<form method="POST"  enctype="multipart/form-data" action="{{action('NovedadController@update', $novedad->id)}}" class="col s12 m8 offset-m2 xl10 offset-xl1">
						...
						<div class="row">
							...
							<div class="input-field col s6">
								...
								<input id="icon_prefix" type="text" class="validate" name="titulo_es" value="{{$novedad->titulo_es}}" >
								<label for="icon_prefix">Título ES</label>
							</div>

							<div class="input-field col s6">
								<i class="material-icons prefix">keyboard_arrow_right</i>
								<input id="icon_prefix" type="text" class="validate" name="titulo_pt" value="{{$novedad->titulo_pt}}" >
								<label for="icon_prefix">Título PT</label>
							</div>
							<div class="input-field col s10">
								<i class="material-icons prefix">keyboard_arrow_right</i>
								<select class="materialSelect" id="categoria_id" name="categoria_id">
									@foreach ($categorias as $c )
									<option value="{{ $c->id }}" @if($c->id == $novedad->categoria_id) selected @endif >{{ ucwords($c->nombre_es) }} (ES) / {{ ucwords($c->nombre_pt) }} (PT) </option>
									@endforeach
								</select>
							</div>
							...
							<div class="input-field col s6">
								...
								<textarea id="texto_es" name="texto_es"> {!! $novedad->texto_es !!} </textarea>
							</div>

							<div class="input-field col s6">

								<textarea id="texto_pt" name="texto_pt"> {!! $novedad->texto_pt !!} </textarea>
							</div>
